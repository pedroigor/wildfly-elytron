...
public interface RealmIdentityCache {
    ...
    /**
     * Puts a new {@link RealmIdentity} into the cache and referenced by the specified {@link IdentityLocator}.
     *
     * @param locator the {@link IdentityLocator} that references the realm identity being cached
     * @param realmIdentity the {@link RealmIdentity} instance
     */
    void put(IdentityLocator locator, RealmIdentity realmIdentity);
     ...
     */
    default RealmIdentity computeIfAbsent(IdentityLocator locator, Function<IdentityLocator, RealmIdentity> mappingFunction) {
        checkNotNullParam("locator", locator);
        checkNotNullParam("mappingFunction", mappingFunction);
        RealmIdentity v;
        if ((v = get(locator)) == null) {
            RealmIdentity newValue;
            if ((newValue = mappingFunction.apply(locator)) != null) {
                put(locator, newValue);
                return newValue;
            }
        }
        return v;
    ...
    /**
     * Obtains a previously cached {@link RealmIdentity} or {@code null} if no entry could be found with the specified {@link IdentityLocator}.
     *
     * @param locator the {@link IdentityLocator} that references a previously cached realm identity
     * @return a cached {@link RealmIdentity} instance or {@code null} if no entry could be found with the specified locator.
     */
    RealmIdentity get(IdentityLocator locator);

    /**
     * Removes a specific cached identity from the cache and referenced by the specified {@link IdentityLocator}.
     *
     * @param locator the {@link IdentityLocator} that references a previously cached realm identity
     */
    void remove(IdentityLocator locator);
