...
class UnixDESCryptPasswordImpl extends AbstractPasswordImpl implements UnixDESCryptPassword {
    ...
    UnixDESCryptPasswordImpl(final SaltedHashPasswordSpec spec) throws InvalidKeySpecException {
        this.salt = ByteBuffer.wrap(spec.getSalt()).getShort();
    ...
    <S extends KeySpec> S getKeySpec(final Class<S> keySpecType) throws InvalidKeySpecException {
        if (keySpecType.isAssignableFrom(SaltedHashPasswordSpec.class)) {
            byte[] salt = ByteBuffer.allocate(2).putShort(this.salt).array();
            return keySpecType.cast(new SaltedHashPasswordSpec(hash.clone(), salt));
        }
